...
@frappe.whitelist(allow_guest=True)
def get_item_groups():
    """Returns the list of item groups."""
    item_groups = []
    # Fetch the list of item groups
    item_group_list = frappe.get_all("Item Group", pluck="name")
    
    for item_group in item_group_list:
        # Fetch detailed information about each item group
        item_groups.append(get_item_group_details(item_group))
    
    return item_groups

def get_item_group_details(item_group_name):
    """Fetches details for a specific item group."""
    item_group = frappe.get_doc("Item Group", item_group_name)
    
    # Print the complete item_group object for debugging
    frappe.errprint(item_group)
    
    # Return only the relevant fields you need
    return {
...
def get_user_details():
    """Fetches and returns details for the logged-in user."""
    ...
    if user == 'Guest':
        return {
            ...
            'user_types': '',
            'customer_info': '',
            'branchs': ''  # Correct field name
        }
    
    user_doc = frappe.get_doc("User", user)
    
    # Accessing fields in the User doctype
    return {
        'username': user_doc.get("username", user),  # Fetching the username
        'user_types': user_doc.get("user_types", ""),
        'customer_info': user_doc.get("customer_info", ""),
        'branchs': user_doc.get("branchs", "")  # Correct field name
    }
...
def get_material_requirements():
    requirements = []
    filters = {"schedule_date": [">=", nowdate()]}  # Filter for new requirements

    if frappe.session.user == "Guest":
        filters.update({"published": 1})

    requirement_list = frappe.get_all("Material Requirement", filters=filters)

    for requirement in requirement_list:
        requirements.append(get_material_requirement_details(requirement.name))

    requirements = categorize_requirements(requirements)
    return requirements

@frappe.whitelist(allow_guest=True)
def get_material_requirement_details(requirement):
    ...
    requirement_details.items = frappe.get_all(
        "Material Requirement Item", filters={"parent": requirement}, fields=["item_group", "qty", "uom", "notes"]
    )
...
def categorize_requirements(requirements):
    ...
    for requirement in requirements:
        if getdate(requirement.schedule_date) >= getdate():
            new_requirements.append(requirement)

    new_requirements.sort(key=lambda x: x.schedule_date)
    return {"upcoming": new_requirements}
